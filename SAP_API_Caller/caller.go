...
import (
	...
	"io/ioutil"
	"net/http"
	sap_api_output_formatter "sap-api-integrations-material-document-reads-rmq-kube/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(materialDocumentYear, materialDocument string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": headerData, "function": "MaterialDocumentHeader"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": itemData, "function": "MaterialDocumentItem"})
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) Item(materialDocumentYear, materialDocument, materialDocumentItem string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": itemData, "function": "Item"})
	if err != nil {
		c.log.Error(err)
		return
	}
